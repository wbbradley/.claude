...
Your job: distill the learning, place it at the right spot in the ontology,
update the ontology, and commit. Keep the tree sane as it grows.
...
- **Intermediate nodes stay lightweight.** `~/wiki/index.md` and any per-topic
  `index.md` are navigation aids, not content. One-line descriptions per
  entry. No prose essays. No duplicated content from leaves. If an index is
  growing, that is a signal to split the sub-tree — not to expand the index.
  Context burn on navigation must stay low.
- **Leaves carry the weight.** The actual knowledge lives in leaf notes.
  Dense, structured, with references. Write for a future reader (often
  you) who has zero thread context.
- **Don't duplicate.** If a relevant note already exists, extend it rather
  than creating a parallel one. Update its `updated:` frontmatter.
- **Kebab-case slugs** for file and directory names. Full words, no
  abbreviations except well-known ones (`llm`, `rag`, `api`).
...
From the current thread, write a concise note. Include:

- **YAML frontmatter**: `title`, `created` (today, YYYY-MM-DD), `updated`
  (same as created on first write), `tags` (list of kebab-case topic tags).
...
2. **Create new branches sparingly.** A new top-level topic is a commitment.
   If the note is the only thing under it, ask: will there plausibly be
   more? If not, file it under an existing branch.
3. **Depth is free, but only up to a point.** Three or four levels is fine
...
executing it**. Show: current shape, proposed shape, which files would
`git mv` where, and which `index.md` entries would update. Only proceed on
approval. Reorganization is cheap when done deliberately, expensive when done
silently.

`git mv` preserves history — always use it instead of delete+create.
...
2. Update `~/wiki/index.md` to list the note — one line, descriptive enough
   ...
   bloat the index.
3. If the sub-tree under the new leaf now has enough notes to deserve its
   own overview, optionally create a `<topic>/index.md` with the same
   lightweight rules (topic name, one paragraph of scope, bullet list of
   child leaves).
...
Use a semantic commit. Examples:
...
Stage explicitly (`git -C ~/wiki add <paths>`), don't `git add -A` — avoids
catching stray files. Then commit. Do not push unless the user asks.

Never add attributions (no `Co-Authored-By`, no "Generated with …").
